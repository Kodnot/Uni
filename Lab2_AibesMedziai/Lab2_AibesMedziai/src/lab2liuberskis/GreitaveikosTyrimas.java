...
public class GreitaveikosTyrimas {
    ...
    private final SortedSetADTx<Student> aSeries = new BstSetKTUx(new Student(), Student.AverageComparator);
    private final SortedSetADTx<Student> aSeries2 = new BstSetKTUx2(new Student());
    private final SortedSetADTx<Student> aSeries3 = new AvlSetKTUx(new Student());
    ...
    public void SisteminisTyrimas() throws InterruptedException {
        try {
            for (int k : TIRIAMI_KIEKIAI) {
                Student[] studMas = StudentGenerator.generateAndShuffle(k, 1.0);
                aSeries.clear();
                aSeries2.clear();
                aSeries3.clear();
                tk.startAfterPause();
                tk.start();
                for (Student a : studMas) {
                    aSeries.add(a);
                }
