package lab2liuberskis.gui.fx;

import lab2liuberskis.gui.MyException;
...
public class Lab2WindowFX extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static final ResourceBundle MESSAGES = ResourceBundle.getBundle("lab2liuberskis.gui.messages");
    ...
    private static SortedSetADTx<Student> studSet;
    ...
    private void treeGeneration(String filePath) throws MyException {
        ...
        if (filePath == null) {
            studArray = StudentGenerator.generateAndShuffle(sizeOfGenSet, sizeOfInitialSubSet, coef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            studSet.load(filePath);
            studArray = new Student[studSet.size()];
            int i = 0;
            for (Object o : studSet.toArray()) {
                studArray[i++] = (Student) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(studArray), new Random());
        }
    ...
    private void treeRemove() {
        ...
        if (studSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(studSet.size());
            Student auto = (Student) studSet.toArray()[nr];
            studSet.remove(auto);
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                studSet = new BstSetKTUx(new Student());
                break;
            case 1:
                studSet = new AvlSetKTUx(new Student());
                break;
            default:
                disableButtons(true);
